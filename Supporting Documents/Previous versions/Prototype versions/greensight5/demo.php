...
<html lang="en-EN">
    <head>
        ...
        <script src="js/calculator.js" type="text/javascript" charset="utf-8">
</script>
    ...
    <body>
        ...
        <div id="dcalclulator" class="shadow corners">
            ...
            <select name="device" id="device" onclick="pickSelect($(sProfileCategory).val(),$(productType).val(), $(brand).val(), $(modelName).val(), $(modelNumber).val(), $(category).val(),$(device).val(), $(rating).val())" size="1" style="display:none">
                <option class="default" value="">
                    Select a Device
                </option>
            </select><br>
            <select name="rating" id="rating" onclick="setQuantity($(sProfileCategory).val(),$(productType).val(), $(brand).val(), $(modelName).val(), $(modelNumber).val(), $(category).val(),$(device).val(), $(rating).val())" size="1" style="display:none">
                <option class="default" value="">
                    Select a Rating
                </option>
            </select><br>
            <select name="onStandby" id="onStandby" onclick="setQuantity($(sProfileCategory).val(),$(productType).val(), $(brand).val(), $(modelName).val(), $(modelNumber).val(), $(category).val(),$(device).val(), $(rating).val())" size="1" style="display:none">
                <option class="default" value="">
                    Select a Rating
                </option>
            </select><br>
            <input type="text" name="quantity" value="1" id="quantity" style="display:none" onenter="getFooprint($(sProfileCategory).val(),$(productType).val(),$(brand).val(),$(modelName).val(), $(modelNumber).val(),$(category).val(),$(device).val(), $(rating).val(), $(this).val())"><br>
            <input type="button" name="reset" value="reset" id="reset" onclick="resetCalc()"><input type="button" name="submit" value="submit" id="submit" onclick="getFooprint($(sProfileCategory).val(),$(productType).val(),$(brand).val(),$(modelName).val(), $(modelNumber).val(),$(category).val(),$(device).val(), $(rating).val(), $(this).val())">
            <div id="dresults">
