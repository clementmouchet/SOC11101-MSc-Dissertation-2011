...
<html lang="en-EN">
    ...
    <body>
        ...
        <div id="dcalclulator" class="shadow corners">
            ...
            </select> <select name="productType" id="productType" class="productType" onclick="pickSelect($(sProfileCategory).val(), $(productType).val(), $(brand).val(), $(modelName).val(), $(modelNumber).val(), $(category).val())" size="1" style="display:none">
                <option id="defaultcat" class="default" value="">
                    Select a product type
                </option>
            </select> <select name="brand" id="brand" onclick="pickSelect($(sProfileCategory).val(),$(productType).val(), $(brand).val(), $(modelName).val(), $(modelNumber).val(), $(category).val())" size="1" style="display:none">
                <option class="default" value="">
                    Select a brand
                </option>
            </select> <select name="modelName" id="modelName" onclick="pickSelect($(sProfileCategory).val(),$(productType).val(), $(brand).val(), $(modelName).val(), $(modelNumber).val(), $(category).val())" size="1" style="display:none">
                <option class="default" value="">
                    Select a model
                </option>
            ...
            </select> <select name="category" id="category" onclick="setQuantity($(sProfileCategory).val(),$(productType).val(), $(brand).val(), $(modelName).val(), $(modelNumber).val(), $(category).val())" size="1" style="display:none">
                <option class="default" value="">
                    Select a category
                </option>
            </select> <select name="device" id="device" onclick="pickSelect($(sProfileCategory).val(),$(productType).val(), $(brand).val(), $(modelName).val(), $(modelNumber).val(), $(category).val(),$(device).val(), $(rating).val())" size="1" style="display:none">
                <option class="default" value="">
                    Select a Device
                </option>
            </select> <select name="rating" id="rating" onclick="setQuantity($(sProfileCategory).val(),$(productType).val(), $(brand).val(), $(modelName).val(), $(modelNumber).val(), $(category).val(),$(device).val(), $(rating).val())" size="1" style="display:none">
                <option class="default" value="">
                    Select a Rating
                </option>
            </select> <select name="onStandby" id="onStandby" onclick="setQuantity($(sProfileCategory).val(),$(productType).val(), $(brand).val(), $(modelName).val(), $(modelNumber).val(), $(category).val(),$(device).val(), $(rating).val())" size="1" style="display:none">
                <option class="default" value="">
                    Select a Rating
                </option>
            </select> <input type="text" name="quantity" value="1" id="quantity" style="display:none" onenter="getFooprint($(sProfileCategory).val(),$(productType).val(),$(brand).val(),$(modelName).val(), $(modelNumber).val(),$(category).val(),$(device).val(), $(rating).val(), $(this).val())"> <input type="button" name="reset" value="reset" id="reset" onclick="resetCalc()"><input type="button" name="submit" value="submit" id="submit" onclick="getFooprint($(sProfileCategory).val(),$(productType).val(),$(brand).val(),$(modelName).val(), $(modelNumber).val(),$(category).val(),$(device).val(), $(rating).val(), $(this).val())">
            <div id="dresults">
