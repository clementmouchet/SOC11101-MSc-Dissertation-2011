	...
	public function getComputerProfile($sProfileCategory, $productType, $brand, $modelName, $modelNumber, $category, $quantity, $device, $rating)
	{
		try {
			...
			$aDeviceValues = array(
				...
				"amount" => round(($aInfo['amount']/$aRequiredProfileItemValues['quantity']),2), // the result is for a single unit, and rounded to 2 decimals
				"unit" => $aInfo['unit'],
    ...
    {		
		try {
			...
			$aDeviceValues = array(
				...
				"amount" => round(($aInfo['amount']/$aRequiredProfileItemValues['numberOwned']),2), // the result is for a single unit, and rounded to 2 decimals
				"unit" => $aInfo['unit'],
				"perUnit" => $aInfo['perUnit'],
				"UID" => $oDataItem->getUID()
			);
